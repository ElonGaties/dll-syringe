...
pub trait Process: AsHandle + AsRawHandle {
    ...
    fn is_alive(&self) -> bool {
        ...
        let result = unsafe { GetExitCodeProcess(self.as_raw_handle().cast(), exit_code.as_mut_ptr()) };
    ...
    fn run_remote_thread<T>(
        ...
        let mut exit_code = MaybeUninit::uninit();
        let result =
            unsafe { GetExitCodeThread(thread_handle.as_raw_handle().cast(), exit_code.as_mut_ptr()) };
        if result == 0 {
            return Err(io::Error::last_os_error());
        }
        debug_assert_ne!(
